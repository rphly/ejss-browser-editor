...
export default class Editor extends Component {
	...
	onSave = () => {
		...
		for (var i = 0; i < variables.length; i++) {
			...
			if (!_.isUndefined(value)) {
				// search and replace
				var re = new RegExp(`(${varName}\\s=\\s)([a-zA-Z0-9]+)(;\\s\\/\\/\\sEjsS\\sModel\\.Variables\\.teachereditsanswershere)`) // regex to search for variable name to be replaced in xhtml
	...
	setModalMeta(doc) {
		if (!_.isNull(doc)) {
			...
			// look for variables
			var re = /\/\/\sEjsS\sModel\.Variables\.teachereditsanswershere.(\S+)/mg
			var match, results = []
